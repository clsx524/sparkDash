  ...
  return (
    <div className="panel" style={{ padding: "var(--density-card-pad)" }}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 border-t border-border pt-3 sm:grid-cols-4 lg:grid-cols-8">
        ...
        {/* Labelled "avg" so it does not read as contradicting the live figure above when the
            cluster is idle and that one has fallen back to zero. */}
        <Stat
          label="Prefill tok/s (avg)"
          value={llm.prefillTpsLifetime != null && llm.prefillTpsLifetime > 0 ? llm.prefillTpsLifetime.toFixed(1) : "—"}
          title="Server-lifetime average: every prompt token admitted over every second spent reaching a first token. Moves slowly, but isn't thrown off by a single slow request."
        />
