...
export function createLivenessState() {
  return {
    ...
    counters: {
      ...
      collectorFallbackSaves: 0, // times recent metric-collection success alone kept a node online
 ...
 * @param {boolean|null} obs.collectorOk    Did any metric domain succeed very recently; null when not applicable.
 * @param {boolean} obs.collectorEligible   Every node may use the collector fallback (not endpoint-dependent).
 * @param {object} [policy]
...
export function applyLivenessObservation(state, obs, policy = {}) {
  ...
  const { sshOk, llmOk, llmEligible, collectorOk, collectorEligible, now } = obs;
  ...
  // The collector signal applies to every node: "a metric domain SSH'd into this host
  // moments ago" is corroborating evidence independent of any served endpoint, so a worker
  // (ineligible for the LLM fallback above) is not left with zero fallback evidence when the
  // dedicated liveness probe alone stalls under heavy load.
  const collectorUsable = collectorEligible && collectorOk === true;
  const externalEvidenceOk = llmUsable || collectorUsable;

  let reason;
  if (sshOk) {
    reason = "ssh-ok";
    state.online = state.consecutiveSshSuccesses >= recoveryNeeded ? true : state.online;
  } else if (externalEvidenceOk) {
    // The host is demonstrably reachable; only the dedicated liveness probe is broken.
    reason = llmUsable ? "llm-fallback" : "collector-fallback";
    if (llmUsable) state.counters.llmFallbackSaves++;
    else state.counters.collectorFallbackSaves++;
    state.online = true;
  } else {
