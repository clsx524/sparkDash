...
test("worker: SSH fails but a metric domain just succeeded -> online, SSH degraded", () => {
  const s = onlineState(1000);
  // Far beyond the grace window, so only the collector fallback can hold it up.
  const r = applyLivenessObservation(s, {
    sshOk: false,
    llmOk: null,
    collectorOk: true,
  ...
  assert.equal(s.online, true);
  assert.equal(r.reason, "collector-fallback");
  assert.equal(s.sshReachable, false);
  assert.equal(s.collectorDegraded, true, "up, but the dedicated liveness probe is not");
  assert.ok(s.counters.collectorFallbackSaves > 0);
});

test("a stale collector reading cannot hold a node online — freshness is the caller's job", () => {
  // collectorOk:false is what SparkMonitor passes once its last successful poll aged out
  // (see _collectorEvidence's COLLECTOR_EVIDENCE_MAX_AGE_MS check).
  const s = onlineState(1000);
  for (let i = 1; i <= 8; i++) {
    applyLivenessObservation(s, {
      sshOk: false,
      llmOk: null,
      collectorOk: false,
      collectorEligible: true,
