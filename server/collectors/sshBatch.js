 ...
 *
 * The exit code is captured into a variable and a bare `echo` forces a newline before the
 * closing marker. Without it, a command whose own stdout does not end in `\n` (e.g.
 * `tailscale status --json`, which prints its closing `}` with no trailing newline) would
 * glue the closing marker onto the command's last line, so parseBatchOutput's line-anchored
 * `startsWith(marker)` check never matches it and the section is reported entirely missing.
 * @param {string[]} cmds
 * @param {string} marker
 */
export function buildBatchScript(cmds, marker) {
  return cmds
    .map((cmd, i) => `echo ${marker} ${i} B; ( ${cmd} ); __c=$?; echo; echo ${marker} ${i} E $__c`)
    .join("; ");
}
