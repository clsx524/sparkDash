...
function fakeRunner(table, opts = {}) {
  ...
  const runner = async (script, timeoutMs) => {
    ...
    const parts = [];
    const re = new RegExp(
      `echo ${marker} (\\d+) B; \\( (.*?) \\); __c=\\$\\?; echo; echo ${marker} \\1 E \\$__c`,
      "g"
    );
    let m;
...
test("buildBatchScript brackets each command and records its exit status", () => {
  ...
  assert.equal(
    s,
    "echo MK 0 B; ( one ); __c=$?; echo; echo MK 0 E $__c; echo MK 1 B; ( two ); __c=$?; echo; echo MK 1 E $__c"
  );
  // Subshells matter: a `cd` or a failing command inside one must not affect the next.
  assert.ok(s.includes("( one )"));
});

test("a command whose own output has no trailing newline does not swallow the closing marker", () => {
  // Reproduces the real bug: `tailscale status --json` prints its closing `}` with no
  // trailing newline, so on a real shell the closing marker used to land on the SAME line
  // as the command's last byte of output (`}__AETOS_..._0_E_0`), and parseBatchOutput's
  // line-anchored startsWith(marker) check never matched it. buildBatchScript's forced
  // `echo` before the closing marker guarantees a newline regardless of the command's own
  // output, so this must parse cleanly — concatenated with NO separator, exactly like a
  // real shell's raw stdout.
  const marker = "MK";
  // `__c=$?; echo; echo MK 0 E $__c` on a real shell prints: "\nMK 0 E 0\n" after the
  // command's own output, with no separator inserted between the command's last byte and
  // the forced blank line.
  const raw = `MK 0 B\n{"a":1}\nMK 0 E 0\n`;
  const r = parseBatchOutput(raw, marker, 1);
  assert.deepEqual(r[0], { ok: true, out: '{"a":1}', code: 0 });
});
